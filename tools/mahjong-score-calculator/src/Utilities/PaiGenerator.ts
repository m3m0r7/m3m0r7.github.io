...
export class PaiGenerator {
  ...
  static generateOneToNine(): { m: PaiName[]; p: PaiName[]; s: PaiName[] } {
    const m = PaiGeneratorCache.getOrSet("oneToNineManzu", () =>
      new PaiGenerator("1", "9", "m").generate(),
    );
    const p = PaiGeneratorCache.getOrSet("oneToNinePinzu", () =>
  ...
  static generateYaoChuHai(): PaiName[] {
    return PaiGeneratorCache.getOrSet("yaoChuHai", () => [
      ...PaiGenerator.generateRoutouHai(),
      ...this.generateJiHai(),
    ]);
  }

  static generateJiHai(): PaiName[] {
    return PaiGeneratorCache.getOrSet("jiHai", () => [
      ...PaiGenerator.generateKazeHai(),
      ...PaiGenerator.generateSangenPai(),
    ]);
  }

  static generateKazeHai(): PaiName[] {
    return PaiGeneratorCache.getOrSet("kazeHai", () =>
      new PaiGenerator("1", "4", "z").generate(),
    );
  }

  static generateSangenPai(): PaiName[] {
    return PaiGeneratorCache.getOrSet("sangenPai", () =>
      new PaiGenerator("5", "7", "z").generate(),
    );
  }
  ...
  static generateChurenPoutou9MenMachi(groupName: PaiGroupName): PaiName[] {
    return PaiGeneratorCache.getOrSet(
      `churenPoutou9MenMachi.${groupName}`,
      () =>
        [
          `1${groupName}`,
          `1${groupName}`,
