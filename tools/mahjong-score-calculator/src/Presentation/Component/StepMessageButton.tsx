...
const StepMessageButton = () => {
  ...
  const doSelectUraDora = () => {
    if (calculationStep?.step === 'select-dora') {
      if (option?.additionalSpecialYaku?.withRiichi || option?.additionalSpecialYaku?.withOpenRiichi || option?.additionalSpecialYaku?.withDoubleRiichi) {
        setCalculationStep?.({
  ...
  if (calculationStep?.step === 'select-hora-pai') {
    return <button type="button" className="button secondary-button w-full" onClick={doSelectUraDora}>
      和了（あがり）牌を選んで点数の結果を確認
    </button>;
  }

  return <button type="button" className="button secondary-button w-full" onClick={doSelectUraDora}>
    {calculationStep?.step === 'select-dora' ? 'ドラ' : '裏ドラ'}を選びおわったら、ここをクリックして次へ
  </button>;
};
