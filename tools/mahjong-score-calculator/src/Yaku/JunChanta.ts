...
export class JunChanta implements Yaku {
  ...
  get isFulfilled(): boolean {
    return this.paiPairCollection.paiPairs.every(
      (paiPair) =>
        (paiPair.isJantou &&
        ...
        (paiPair.isShuntsu &&
          PaiGenerator.generatePenchanHai().some((paiNames) =>
            paiPair.pattern.includesWithMatrix(paiNames, "AND"),
          )) ||
        ((paiPair.isKoutsu || paiPair.isToitsu) &&
          paiPair.pattern.includesWithMatrix(PaiGenerator.generateRoutouHai())),
    );
  }
}
