...
export class SanShokuDouKou implements Yaku {
  ...
  get isFulfilled(): boolean {
    for (let i = 0; i < this.paiPairCollection.paiPairs.length; i++) {
      ...
      if (
        this.paiPairCollection.containsKoutsuOrKan(koutsuManzu) &&
        this.paiPairCollection.containsKoutsuOrKan(koutsuPinzu) &&
        this.paiPairCollection.containsKoutsuOrKan(koutsuSouzu)
      ) {
        return true;
      }
    }
