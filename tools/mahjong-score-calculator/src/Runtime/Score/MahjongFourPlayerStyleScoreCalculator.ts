...
export class MahjongFourPlayerStyleScoreCalculator {
  ...
  private calculateScoreData(
    collectionAndScores: CollectionAndScores[],
  ): ScoreData | null {
    ...
    for (const collectionAndScore of collectionAndScores) {
      ...
      if (calculationYaku === "DOUBLE_FULL") {
        // NOTE: Double Yakuman
        baseScore += isParent ? 96000 : 64000;
        tempScoreData.fu = null;
        tempScoreData.appliedFuList = [];
      } else if (calculationYaku === "FULL") {
        // NOTE: Yakuman
        baseScore += isParent ? 48000 : 32000;
        tempScoreData.fu = null;
        tempScoreData.appliedFuList = [];
      } else if (calculationYaku >= 11 && calculationYaku <= 12) {
        // NOTE: Normally scoring
        baseScore += isParent ? 36000 : 24000;
      } else if (calculationYaku >= 8 && calculationYaku <= 10) {
        // NOTE: Normally scoring
        baseScore += isParent ? 24000 : 16000;
      } else if (calculationYaku >= 6 && calculationYaku <= 7) {
        // NOTE: Normally scoring
        ...
        // NOTE: under 4 yaku
        baseScore +=
          this.scoreTable?.[isParent ? "parent" : "child"]?.[calculationYaku]?.[
            tempScoreData.fu ?? 30
          ] ?? 0;
      }
